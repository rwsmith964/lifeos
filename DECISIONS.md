...
**Verification:** `pnpm typecheck && pnpm lint && pnpm test && pnpm build` all pass (262/262 — no new automated test added; this is a straightforward query-scoping fix with an existing, already-tested-elsewhere helper, and the realistic regression risk (two real households both having a gift suggestion due in the same 14-day window) isn't easily reproducible in a unit test without a lot of fixture setup for low marginal value). Live re-verification (regenerate Richard's brief again post-deploy and confirm "Carol Smith" no longer appears) pending for next round.

**Blast radius / what to double check going forward:** this specific bug only affected the *brief's* gift-reminder section. Nothing else in `generateDailyBrief` had the same gap. Worth a quick sweep of any other service-role-executed queries added in the future for the same "list() with no household_id filter" pattern — see KNOWN-ISSUES.md.
